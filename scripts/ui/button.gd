...
@export var ui_style_box : UIStyleBox
...
func _ready() -> void:
	...
	pressed.connect(func(): if Global.sound_manager: Global.sound_manager.play("MouseClickSound"))
...
func hover() -> void:
	...
	if Global.sound_manager:
		Global.sound_manager.play("ChutterClickSound")
	
	if ui_style_box:
		ui_style_box.active = true
